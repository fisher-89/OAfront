...
export default class Textinput extends PureComponent{
    render(){
      ...
      const typeList = ["file","text","date","datetime","time","file","array","select","department","staff","shop","region","api"];
      if(type==="text" || type==="int" || type==="array"){
        return <div className={style.textTag}>
      ...
      if(type==="department" || type==="date" || type==="datetime" || type==="time" || type==="api" || type==="shop" || type==="select" || type==="staff" ||type==="region"){
        return <div className={style.textTag}>
          ...
          <div className={style.tagcontainer}>
            ...
            <i className={style.selectIcon}></i>
          </div>
        </div>
      }
      if(type==="file"){
        return <div className={style.textTag}>
                  <div className={style.tagname}>{data.name}</div>
                  <div className={style.tagcontainer}>
                    <div className={style.fileupload}><i></i></div>
                    <div className={style.fileupload}><i></i></div>
                    <div className={style.fileupload}><i></i></div>
                    <div className={style.fileupload} style={{marginRight:'0'}}><i></i></div>
                  </div>
              </div>
      }
      if(type===undefined){
        return <div className={style.controlTag}>
                {data.name}
                <i className={style.controlIcon}></i>
                </div>
      }
    }
