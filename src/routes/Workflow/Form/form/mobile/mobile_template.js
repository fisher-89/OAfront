...
class MobileTemplate extends Component {
  ...
  handleDragStart = (data, startPoint, tagPosition,grid = null,extraparams={}) => {
    ...
    const formData = gridIndex===null ? makeTagarray(fields,grids) : makeTagarray(grids[gridIndex].fields);
  ...
  handleDragConfirm = (data) => {
    let { form, fields, grids } = this.props;
    const { dataIndex, parentGridIndex } = this.state;
    ...
    if(parentGridIndex !== null){
      let results = handleChildsort(data,grids,parentGridIndex);
      grids = results.grids;
    }else{
  ...
  handleElemove = (index,ymove,currentDirection,currentIndex=null,dataindex=null) => {
    this.setState({
      ...
      dataindex:dataindex //data.index
    })
  }
  ...
  makeChildtemplate = ()=>{
    ...
    return <Tabs className={styles.childtemplatetabs}  tabPosition="left"
                 tabBarGutter={4} {...activeKeyoption}
