  ...
  render() {
    ...
    return (
      <div className="ant-table-filter-dropdown ant-table-tree-filter">
        ...
        <div className="scroll-bar" style={{ maxHeight: 208, padding: '0 10px' }}>
          <Checkbox.Group
            value={value}
            ...
            {filters.map(item =>
              (
                <div key={item.value} style={{ marginTop: 10 }}>
                  <Checkbox disabled={item.disabled === true} value={item.value} style={{ whiteSpace: 'nowrap' }}>
                    {item.text}
                  </Checkbox>
                </div>
              ))
        ...
        <div className="ant-table-filter-dropdown-btns">
          <a className="ant-table-filter-dropdown-link confirm" onClick={() => handleConfirm(value)}>确定</a>
          <a className="ant-table-filter-dropdown-link clear" onClick={() => this.handleReset()}>重置</a>
        </div>
      </div>
    );
  }
